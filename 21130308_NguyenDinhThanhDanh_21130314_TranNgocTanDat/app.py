...
def build_model_by_nn(X_transformed, y):
    model = Sequential()
    model.add(Dense(128, input_dim=X_transformed.shape[1], activation='relu'))
    model.add(Dense(64, activation='relu'))
    model.add(Dense(32, activation='relu'))
    model.add(Dense(1, activation='linear'))
    model.compile(optimizer='adam', loss='mean_squared_error')
    model.fit(X_transformed, y, epochs=100, batch_size=32, verbose=0)
    return model
